...
public class FileHandler
    ...
    /**
     * The count number of recent files to keep when rotating
     */
    ...
    private void configure() {
        ...
        if (count == null)
            count = getProperty(className + ".count", 0);
        if (suffix == null)
        ...
        if (formatterName != null) {
            try {
                setFormatter((Formatter) cl.loadClass(formatterName).getDeclaredConstructor().newInstance());
            } catch (Exception e) {
    ...
    private Integer getProperty(String name, int defaultValue) {
        String p = getProperty(name, null);
        Integer result = null;
        try {
            if (p != null)
    ...
    protected void openWriter() {
        ...
        try {
            ...
            // discard older rotated files
            if (count > 0 && rotatable && !prefix.equals("")) {
                File[] files = parent.listFiles();
                Arrays.sort(files);
                for (int i=files.length-1, c=count; i>=0; i--) {
                    File f = files[i];
                    if (f.getName().startsWith(prefix) && f.getName().endsWith(suffix) && --c < 0)
                        f.delete();
                }
            }
        } catch (Exception e) {
